...
public class ThirdServiceCallProcessor {
	...
	public void process(Exchange exchange) throws Exception {
		...
		Long sleep = 0L; 
		try {
			sleep = Long.parseLong(exchange.getIn().getHeader("SLEEP", String.class));
		} catch(Exception ex)	{ }
		
		ServiceResponse serveResp = restServiceCaller.callNonEurekaSCIntIV(sleep);
		
		exchange.getIn().setHeader(SpringCamelConstant.HEADER_ROUTE_THIRD_RESPONSE, serviceResponse);
		exchange.getIn().setHeader(SpringCamelConstant.HEADER_ROUTE_SLEEP_RESPONSE, serveResp);
